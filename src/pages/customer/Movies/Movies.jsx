...
const getFilteredMovies = ({ movies, keyword, activeFilter, sortType }) => {
  ...
  if (sortType === "date") {
    return [...result].sort(
      (a, b) =>
        new Date(b.ngayKhoiChieu || 0) -
        new Date(a.ngayKhoiChieu || 0),
    );
  }
...
const MovieRow = ({
  ...
  const totalSlides = Math.max(
    1,
    Math.ceil(movies.length / MOVIES_PER_ROW),
  );
    ...
    <section className="mt-14">
      ...
      <div className="relative">
        ...
        <div className="flex gap-5 overflow-x-auto pb-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {isLoading
            ? Array.from({ length: MOVIES_PER_ROW }).map(
                (_, index) => (
...
const Movies = () => {
  ...
  const availableMovies = useMemo(
    () =>
      movies.filter(
        (movie) => !movie.sapChieu,
      ),
    [movies],
  );

  const { hotMovies, showingMovies } = useMemo(
    () => getCategoryMovies(availableMovies),
    [availableMovies],
  );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredMovies.length / pageSize),
  );
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_330px] lg:items-end">
            <div>
              ...
              <p className="mt-4 max-w-2xl text-base font-medium leading-7 text-zinc-300">
                Tìm kiếm phim, lọc theo trạng thái
                và phân trang danh sách phim ngay
                trên giao diện.
              </p>
            </div>
