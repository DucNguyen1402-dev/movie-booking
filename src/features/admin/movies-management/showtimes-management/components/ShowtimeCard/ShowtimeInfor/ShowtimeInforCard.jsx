...
const {date, time} = formatShowtime(ngayChieuGioChieu);
  ...
  return (
    <article className="space-y-5 rounded-xl border border-slate-600 bg-linear-to-br from-slate-700/60 to-slate-700 p-5 text-slate-100 transition-transform duration-300 hover:-translate-y-2 hover:shadow-md hover:ring-2 hover:ring-indigo-500/20 hover:border-indigo-500 group"
    ...
    >
      ...
      <div className="space-y-3 text-sm">
        <div className="flex items-center gap-2 text-slate-200 group-hover:text-slate-100">
          ...
          <span className ="group-hover:text-rose-400">{date} - {time}</span>
        </div>

        <div className="flex items-center gap-2">
          <Ticket className="size-4 text-slate-200 group-hover:text-slate-100" />

          <span className="ml-1 group-hover:text-rose-400">{giaVe.toLocaleString("vi-VN", {style: "currency", currency: "VND"})}</span>
        </div>
